...
function PhysicalInvoiceDocument({ invoice, business }: { invoice: any; business: any }) {
  ...
  const tax = invoice.snapshot?.tax;
  const exchangeCredit = Number(invoice.snapshot?.exchange_credit || 0);
  const exchange = invoice.snapshot?.exchange;
  return <article className="invoice-print-document" data-invoice-id={invoice.id}>
    ...
    <div className="invoice-bottom"><section className="invoice-payments"><h2>Payment</h2>{exchangeCredit > 0 && <p><span>Old gold exchange credit</span><b>{formatMoney(exchangeCredit)}</b></p>}{invoice.payments.map((payment: any, index: number) => <p key={`${payment.method}-${index}`}><span>{payment.method}{payment.reference ? ` · ${payment.reference}` : ''}</span><b>{formatMoney(payment.amount)}</b></p>)}<p className="paid"><span>Total settled</span><b>{formatMoney(invoice.amount_paid)}</b></p><p><span>Balance</span><b>{formatMoney(invoice.balance)}</b></p></section><section className="invoice-totals"><h2>Invoice summary</h2>{price.metal_value != null && <p><span>Metal value</span><b>{formatMoney(price.metal_value)}</b></p>}{price.making_charge != null && <p><span>Making charges</span><b>{formatMoney(price.making_charge)}</b></p>}{price.wastage_value != null && <p><span>Wastage</span><b>{formatMoney(price.wastage_value)}</b></p>}{price.other_charges != null && Number(price.other_charges) > 0 && <p><span>Other charges</span><b>{formatMoney(price.other_charges)}</b></p>}{price.discount != null && Number(price.discount) > 0 && <p><span>Discount</span><b>− {formatMoney(price.discount)}</b></p>}{tax ? <><p><span>Taxable value</span><b>{formatMoney(tax.taxable_value)}</b></p>{tax.tax_type === 'INTER_STATE' ? <p><span>IGST{tax.gst_rate_percent != null ? ` @ ${tax.gst_rate_percent}%` : ''}</span><b>{formatMoney(tax.igst)}</b></p> : <><p><span>CGST{tax.gst_rate_percent != null ? ` @ ${Number(tax.gst_rate_percent) / 2}%` : ''}</span><b>{formatMoney(tax.cgst)}</b></p><p><span>SGST{tax.gst_rate_percent != null ? ` @ ${Number(tax.gst_rate_percent) / 2}%` : ''}</span><b>{formatMoney(tax.sgst)}</b></p></>}<p><span>Total GST</span><b>{formatMoney(tax.total_gst)}</b></p></> : <>{price.subtotal != null && <p><span>Subtotal</span><b>{formatMoney(price.subtotal)}</b></p>}{price.tax_amount != null && <p><span>GST</span><b>{formatMoney(price.tax_amount)}</b></p>}</>}<p className="invoice-grand"><span>Jewellery total</span><b>{formatMoney(invoice.total)}</b></p>{exchangeCredit > 0 && <p><span>Old gold credit</span><b>− {formatMoney(exchangeCredit)}</b></p>}</section></div>
    {exchange && <section className="invoice-parties"><div><small>GOLD EXCHANGE SETTLEMENT · {exchange.exchange_number}</small><span>Calculated old gold value · {formatMoney(exchange.calculated_gold_value)}</span><span>Agreed exchange value · {formatMoney(exchange.agreed_exchange_value)}</span><b>{exchange.payment_direction === 'CUSTOMER_TO_SHOP' ? 'CUSTOMER PAID SHOP' : exchange.payment_direction === 'SHOP_TO_CUSTOMER' ? 'SHOP PAID CUSTOMER' : 'BALANCED'} · {formatMoney(exchange.difference)}</b>{exchange.settlement_method && <span>{exchange.settlement_method.replaceAll('_', ' ')}{exchange.settlement_reference ? ` · ${exchange.settlement_reference}` : ''}{exchange.settlement_number ? ` · ${exchange.settlement_number}` : ''}</span>}</div></section>}
