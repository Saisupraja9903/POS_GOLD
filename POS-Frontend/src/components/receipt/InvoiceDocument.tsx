...
export function InvoiceDocument({ invoice, business }: { invoice: any; business: any }) {
  ...
  return <article className="invoice-print-document" data-invoice-id={invoice.id}>
    ...
    <section className="invoice-meta"><div><small>INVOICE DATE</small><b>{date.toLocaleDateString('en-IN', { dateStyle: 'medium' })}</b></div><div><small>TIME</small><b>{date.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</b></div><div><small>BRANCH</small><b>{invoice.branch}</b></div>{invoice.salesperson_id && <div><small>SALES PERSON</small><b>{invoice.salesperson_id}</b></div>}</section>
