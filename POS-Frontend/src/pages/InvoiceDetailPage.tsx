...
export function InvoiceDetailPage() {
  const { id } = useParams(), navigate = useNavigate(), location = useLocation(), autoPrinted = useRef(false);
  ...
  useEffect(() => { autoPrinted.current = false; }, [id]);
  useEffect(() => { if (invoice && business && location.state?.printReceipt && !autoPrinted.current) { autoPrinted.current = true; requestAnimationFrame(print); } }, [invoice, business, location.state]);
