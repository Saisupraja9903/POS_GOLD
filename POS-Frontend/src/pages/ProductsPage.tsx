...
export function ProductsPage() {
  ...
  useEffect(() => {
    void api.get('/pos/cart/active').then(({ data }) => setInCartIds(new Set(data.lines.map((line: { item_id: string }) => line.item_id))));
  }, []);

  async function addToBill(item: PosProduct) {
    if (pendingAdds.current.has(item.id)) return;
    pendingAdds.current.add(item.id);
    setAdding(item.id);
    setError('');
    try {
      await api.post('/pos/cart/items', null, { params: { item_id: item.id } });
      setInCartIds(current => new Set(current).add(item.id));
      await loadProducts();
    } catch (reason: any) {
      if (reason.response?.status === 409 || reason.response?.status === 404) {
        if (reason.response?.data?.error?.code === 'ITEM_ALREADY_IN_CART') setInCartIds(current => new Set(current).add(item.id));
        setError(reason.response?.data?.error?.message || 'This item is no longer available.');
        await loadProducts();
