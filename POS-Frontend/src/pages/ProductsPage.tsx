...
export function ProductsPage({ canAddToBill = true }: { canAddToBill?: boolean }) {
  ...
  useEffect(() => { if (!canAddToBill) return; void api.get('/pos/cart/active').then(({ data }) => { const counts = data.lines.reduce((all: Record<string, number>, line: { product_id: string }) => ({ ...all, [line.product_id]: (all[line.product_id] || 0) + 1 }), {}); setCartCounts(counts); setInCartIds(new Set(Object.keys(counts))); }); }, [canAddToBill]);
  ...
  return <Page title="Available products" subtitle="Currently sellable jewellery in your branch.">
    ...
    {error && !data.items.length ? <ErrorState text={error} retry={loadProducts} /> : data.items.length ? <><div className="available-table"><header><span>PRODUCT</span><span>BARCODE</span><span>PURITY</span><span>GROSS</span><span>NET</span><span>FINE</span><span>WASTAGE</span><span>AVAILABLE STOCK</span>{canAddToBill&&<span>ACTION</span>}</header>
      {data.items.map(item => { const remaining = item.available_count - (cartCounts[item.product_id] || 0); const wastage = item.wastage_percent === null || item.wastage_percent === undefined ? 'Not configured' : `${item.wastage_percent}%`; return <article key={item.product_id}><span><b>{item.name}</b><small>{item.sku || item.metal}</small></span><span>{item.barcode}</span><span>{item.purity} / {Number(item.fineness).toFixed(0)}</span><span>{item.gross_weight} g</span><span>{item.net_weight} g</span><span>{item.fine_weight} g</span><span>{wastage}</span><mark className="available">{item.availability_status.replaceAll('_', ' ')}<small>{item.available_count} PIECES</small></mark>{canAddToBill&&(selecting === item.product_id ? <div className="quantity-picker"><button aria-label="Decrease quantity" disabled={quantity <= 1} onClick={() => setQuantity(value => value - 1)}><Minus /></button><b>{quantity}</b><button aria-label="Increase quantity" disabled={quantity >= remaining} onClick={() => setQuantity(value => value + 1)}><Plus /></button><button disabled={adding === item.id} onClick={() => void addToBill(item)}><ShoppingBag />{adding === item.id ? 'Adding…' : `Add ${quantity} to Bill`}</button></div> : <button disabled={remaining < 1} onClick={() => { setSelecting(item.product_id); setQuantity(1); }}><ShoppingBag />{remaining < 1 ? 'All in cart' : inCartIds.has(item.product_id) ? 'Add more' : 'Add to Bill'}</button>)}</article>; })}</div>
