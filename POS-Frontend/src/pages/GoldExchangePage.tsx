...
export function GoldExchangePage() {
  ...
  if (result) return <CompletedExchange result={result} invoice={invoice} customer={selectedCustomer} oldGold={oldGold} purity={selectedPurity} items={selectedItems} onNew={startNewExchange} onInvoice={() => navigate(`/invoices/${result.invoice_id}`)} onPrint={() => navigate(`/invoices/${result.invoice_id}`, { state: { printReceipt: true } })} />;
...
function CompletedExchange({ result, invoice, customer, oldGold, purity, items, onNew, onInvoice, onPrint }: any) {
  ...
  return <Page title="Gold Exchange" subtitle={`Exchange No. ${result.exchange_number}`}><div className="gold-exchange exchange-completed">
    ...
    <div className="exchange-completion-grid"><main className="grid gap-4"><section className="completion-card old-gold-details"><small>WHAT THE CUSTOMER GAVE</small><h2>Old Gold</h2><h3>{oldGold.description}</h3><div className="fact-grid"><Fact label="Gross weight" value={weight(oldGold.gross_weight)} /><Fact label="Stone / non-gold" value={weight(oldGold.stone_weight)} /><Fact label="Net weight" value={weight(netGold)} /><Fact label="Purity" value={purity?.label || 'Tested purity'} /></div><div className="credit-strip"><span>Exchange credit</span><b>{money(result.exchange_credit)}</b></div></section>
      ...
      <aside className="completion-side"><section className="completion-card settlement-card"><small>SETTLEMENT</small><Fact label="New jewellery total" value={money(result.new_total)} /><Fact label="Exchange credit" value={`−${money(result.exchange_credit)}`} /><Fact label="Approved discount" value={amount(snapshot.discount) ? `−${money(snapshot.discount)}` : '—'} /><Fact label="Tax" value={amount(snapshot.tax_amount) ? money(snapshot.tax_amount) : 'Included in invoice'} /><hr />{refundDue ? <><small>REFUND DUE</small><strong className="settlement-total warning">{money(refundDue)}</strong><p>The exchange is recorded, but this amount is still due to the customer.</p></> : balanceDue ? <><small>BALANCE PAYABLE</small><strong className="settlement-total">{money(balanceDue)}</strong><p>Collect the remaining amount through the normal payment workflow.</p></> : <><small>SETTLEMENT</small><strong className="settlement-total success">Fully settled</strong><p>✓ Payment complete</p></>}</section>
