...
export function GoldExchangePage() {
  ...
  const [oldGold, setOldGold] = useState({ description: 'Old gold jewellery', gross_weight: '', stone_weight: '0', purity_id: '', agreed_exchange_value: '' });
  ...
  useEffect(() => {
    if (!customer || !oldGold.gross_weight || !oldGold.purity_id || !oldGold.agreed_exchange_value || !selectedItems.length) { setPreview(null); return; }
    const timer=setTimeout(async()=>{try{const {data}=await api.post('/pos/exchanges/preview',{customer_id:customer,gross_weight:oldGold.gross_weight,stone_weight:oldGold.stone_weight,purity_id:oldGold.purity_id,agreed_exchange_value:oldGold.agreed_exchange_value,new_item_ids:selectedItems.map(item=>item.id)});setPreview(data);setError('')}catch(e:any){setPreview(null);setError(e.response?.data?.error?.message||'Unable to calculate the exchange.')}},300);return()=>clearTimeout(timer);
  },[customer,oldGold.gross_weight,oldGold.stone_weight,oldGold.purity_id,oldGold.agreed_exchange_value,items]);
  ...
  async function completeExchange() {
    ...
    try {
      const response = await api.post('/pos/exchanges', {
        ...
        purity_id: oldGold.purity_id, agreed_exchange_value: oldGold.agreed_exchange_value, new_item_ids: selectedItems.map(item => item.id), payments: [], verification,
      });
      setResult(response.data);
      if (response.data.invoice_id) try { setInvoice((await api.get(`/pos/invoices/${response.data.invoice_id}`)).data); } catch { /* Settlement still renders from the authoritative exchange response. */ }
    } catch (requestError: any) {
  ...
  function startNewExchange() {
    setItems([]); setCustomer(''); setCode(''); setLookupItem(null); setOldGold({ description: 'Old gold jewellery', gross_weight: '', stone_weight: '0', purity_id: '', agreed_exchange_value: '' });
  ...
  const canContinue = customer && oldGold.gross_weight && oldGold.purity_id && oldGold.agreed_exchange_value && selectedItems.length && preview;

  if (result) return <CompletedExchange result={result} invoice={invoice} customer={selectedCustomer} oldGold={oldGold} purity={selectedPurity} items={selectedItems} onNew={startNewExchange} onInvoice={() => result.invoice_id && navigate(`/invoices/${result.invoice_id}`)} onPrint={() => result.invoice_id && navigate(`/invoices/${result.invoice_id}`, { state: { printReceipt: true } })} onPaymentCompleted={async (payment: any) => { const invoiceId=payment.invoice_id||result.invoice_id; setResult((current: any) => ({ ...current, invoice_id:invoiceId, invoice_number:payment.invoice_number||current.invoice_number, status: payment.exchange_status, balance: '0', refund_due: '0', settlement:payment })); try { setInvoice((await api.get(`/pos/invoices/${invoiceId}`)).data); } catch { /* The settlement response remains authoritative. */ } }} />;
  ...
  return <Page title="Gold Exchange" subtitle="Enter details, review, verify, then complete."><div className="gold-exchange"><header className="exchange-header"><div><small>GOLD EXCHANGE</small><h2>{step === 'ENTRY' ? 'Enter exchange details' : step === 'VERIFY' ? 'Verify & approve exchange' : 'Gold exchange approved'}</h2><p>Customer → Old Gold → New Jewellery → Confirm → Verify → Approve → Complete</p></div><span className="exchange-status">{step}</span></header><div className="grid gap-4 xl:grid-cols-[1.5fr_.8fr]"><main className="grid gap-4">
    {step === 'ENTRY' && <section className="exchange-card"><small>AGREED OLD GOLD EXCHANGE VALUE</small><input type="number" min="0.01" step="0.01" value={oldGold.agreed_exchange_value} onChange={event=>setOldGold({...oldGold,agreed_exchange_value:event.target.value})} placeholder="₹ Shop-agreed exchange value"/><p>This value is entered by the Sales Person. The backend separately calculates and preserves the gold valuation.</p></section>}
  ...
  </main><aside className="exchange-summary"><small>EXCHANGE SUMMARY</small><h2>{step === 'APPROVED' ? 'Ready to confirm' : 'Details review'}</h2><p>New jewellery selected: <b>{selectedItems.length}</b></p>{preview&&<><Fact label="Calculated gold value" value={money(preview.calculated_gold_value)}/><Fact label="Agreed exchange value" value={money(preview.agreed_exchange_value)}/><Fact label="New jewellery taxable" value={money(preview.taxable_value)}/><Fact label="GST" value={money(preview.total_gst)}/><Fact label="New jewellery total" value={money(preview.new_jewellery_total)}/><hr/><small>{preview.payment_direction==='CUSTOMER_TO_SHOP'?'CUSTOMER PAYS':preview.payment_direction==='SHOP_TO_CUSTOMER'?'SHOP PAYS CUSTOMER':'EXCHANGE BALANCED'}</small><strong className="settlement-total">{money(preview.difference)}</strong></>}{error && <p className="exchange-error">{error}</p>}{step === 'APPROVED' && <button disabled={busy||!preview} onClick={completeExchange}>{busy ? 'Confirming…' : 'Confirm Gold Exchange'}</button>}</aside></div></div></Page>;
}
...
function CompletedExchange({ result, invoice, customer, oldGold, purity, items, onNew, onInvoice, onPrint, onPaymentCompleted }: any) {
  ...
  const displayItems = invoice?.items || result.items || items;
  return <Page title="Gold Exchange" subtitle={`Exchange No. ${result.exchange_number}`}><div className="gold-exchange exchange-completed">
    ...
    <div className="exchange-completion-grid"><main className="grid gap-4"><section className="completion-card old-gold-details"><small>WHAT THE CUSTOMER GAVE</small><h2>Old Gold</h2><h3>{oldGold.description}</h3><div className="fact-grid"><Fact label="Gross weight" value={weight(oldGold.gross_weight)} /><Fact label="Stone / non-gold" value={weight(oldGold.stone_weight)} /><Fact label="Net weight" value={weight(netGold)} /><Fact label="Purity" value={purity?.label || 'Tested purity'} /></div><div className="credit-strip"><span>Exchange credit</span><b>{money(result.exchange_credit)}</b></div></section>
      ...
      <section className="completion-card inventory-result"><small>EXCHANGE INVENTORY</small><div><b>Old gold</b><span>{settled ? 'Received · Exchange In' : 'Pending settlement · Not received'}</span><em>{result.exchange_number}</em></div><div><b>New jewellery</b><span>{settled ? 'Sold · Exchange Out' : 'Reserved · Not sold'}</span><em>{items.map((item: any) => item.tag_number).filter(Boolean).join(', ') || 'Serialised items'}</em></div></section></main>
      <aside className="completion-side"><section className="completion-card settlement-card"><small>SETTLEMENT</small><Fact label="Taxable value" value={money(tax.taxable_value)} />{tax.gst_rate_percent != null && <Fact label={`GST @ ${tax.gst_rate_percent}%`} value={money(tax.total_gst)} />}{tax.tax_type === 'INTER_STATE' ? <Fact label={`IGST${tax.gst_rate_percent != null ? ` @ ${tax.gst_rate_percent}%` : ''}`} value={money(tax.igst)} /> : <><Fact label={`CGST${tax.gst_rate_percent != null ? ` @ ${amount(tax.gst_rate_percent) / 2}%` : ''}`} value={money(tax.cgst)} /><Fact label={`SGST${tax.gst_rate_percent != null ? ` @ ${amount(tax.gst_rate_percent) / 2}%` : ''}`} value={money(tax.sgst)} /><Fact label="Total GST" value={money(tax.total_gst)} /></>}<Fact label="New jewellery total" value={money(result.new_total)} /><Fact label="Exchange credit" value={`−${money(result.exchange_credit)}`} /><Fact label="Approved discount" value={amount(snapshot.discount) ? `−${money(snapshot.discount)}` : '—'} /><hr />{refundDue ? <><small>REFUND DUE</small><strong className="settlement-total warning">{money(refundDue)}</strong><p>The exchange is recorded, but this amount is still due to the customer.</p></> : balanceDue ? <><small>BALANCE PAYABLE</small><strong className="settlement-total">{money(balanceDue)}</strong><p>Collect the remaining amount through the normal payment workflow.</p></> : <><small>SETTLEMENT</small><strong className="settlement-total success">Fully settled</strong><p>✓ Payment complete</p></>}</section>
        ...
        {balanceDue > 0 && <ExchangePayment exchange={result} outstanding={balanceDue} direction="CUSTOMER_TO_SHOP" onCompleted={onPaymentCompleted} />}
        {refundDue > 0 && <ExchangePayment exchange={result} outstanding={refundDue} direction="SHOP_TO_CUSTOMER" onCompleted={onPaymentCompleted} />}
...
function ExchangePayment({ exchange, outstanding, direction, onCompleted }: any) {
  const isRefund = direction === 'SHOP_TO_CUSTOMER';
  ...
  function beginPayment() {
    setError('');
    if (!Number.isFinite(due) || due <= 0) { setError('There is no settlement amount to record.'); return; }
    if (method !== 'CASH' && !reference.trim()) { setError('Enter the transaction reference.'); return; }
  ...
  async function confirmPayment() {
    ...
    try {
      const endpoint = isRefund ? 'refund' : 'payments';
      const response = await api.post(`/pos/exchanges/${exchange.id}/${endpoint}`, { method, reference: reference.trim() || null, idempotency_key: idempotencyKey });
      setReceived({ ...response.data, amount: due, method }); setConfirming(false); setIdempotencyKey(''); await onCompleted(response.data);
    } catch (requestError: any) { setError(requestError.response?.data?.error?.message || `The ${isRefund ? 'payout' : 'payment'} could not be completed. Please try again.`); setConfirming(false); }
    finally { setSubmitting(false); }
  }
  if (received) return <section className="completion-card exchange-payment received"><small>{isRefund ? 'SHOP PAID CUSTOMER' : 'CUSTOMER PAID SHOP'}</small><b>{money(received.amount)}</b><span>{received.method.replaceAll('_', ' ')}</span><span>{received.payment_number}</span><mark>PAID</mark></section>;
  const methods = isRefund ? [['CASH', 'Cash'], ['UPI', 'UPI'], ['BANK_TRANSFER', 'Bank Transfer']] : [['CASH', 'Cash'], ['UPI', 'UPI'], ['CARD', 'Card'], ['BANK_TRANSFER', 'Bank Transfer']];
  return <section className="completion-card exchange-payment"><small>{isRefund ? 'SHOP PAYS CUSTOMER' : 'CUSTOMER PAYS'}</small><div className="payment-methods">{methods.map(([value, label]) => <button className={method === value ? 'active' : ''} key={value} onClick={() => setMethod(value)}>{label}</button>)}</div><label>Amount<input type="text" readOnly value={money(outstanding)} /></label>{method !== 'CASH' && <label>Transaction reference<input value={reference} onChange={event => setReference(event.target.value)} placeholder="UPI / bank / card reference" /></label>}{error && <p className="payment-error">{error}</p>}<button className="payment-submit" disabled={submitting} onClick={beginPayment}>{isRefund ? 'Record Customer Payout' : 'Proceed to Payment'}</button>{confirming && <div className="payment-confirm-backdrop" role="dialog" aria-modal="true"><section><small>{isRefund ? 'CONFIRM CUSTOMER PAYOUT' : 'CONFIRM PAYMENT'}</small><h3>{money(due)}</h3><p>{method.replaceAll('_', ' ')}</p><span>{isRefund ? 'Confirm that the shop has paid the displayed amount to the customer.' : 'Confirm that the customer has paid the displayed amount.'}</span><div><button className="secondary-action" disabled={submitting} onClick={() => setConfirming(false)}>Back</button><button className="primary-action" disabled={submitting} onClick={confirmPayment}>{submitting ? 'Processing…' : isRefund ? 'Confirm Payout' : 'Confirm Payment'}</button></div></section></div>}</section>;
}
